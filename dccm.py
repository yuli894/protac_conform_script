...
class Traj:
    ...
    def getCoord(self):
        ...
        tmp = pd.concat([(pd.DataFrame(n_series[i], columns=[
            [str(n_series.keys()[i]), str(n_series.keys()[i]), str(n_series.keys()[i])], ["x", "y", "z"]])) for i in
                         range(n_series.size)], axis=1)
        print("input file contains {} frames  from {} ps to {} ps dt {}ps".format(int(tmp.shape[1] / 3),
                                                                                  n_series.keys()[0],
                                                                                  n_series.keys()[-1],
                                                                                  float(n_series.keys()[1]) - float(
                                                                                      n_series.keys()[0])))
        # print(tmp)
        return tmp
...
class gro:
    ...
    def readGro(self):
        ...
        for line in gro.readlines():
            ...
            if tmp != []:
                atomSet.append(tmp)
        return pd.DataFrame(atomSet,
                            columns=["resNum", "resName", "atomName", "atomNum", "X", "Y", "Z", "Vx", "Vy", "Vz"])
...
plt.colorbar()
plt.contour(x, y, z, colors="grey", levels=[-1, -0.75, -0.5, -0.25, 0.25, 0.5, 0.75, 1], linewidths=0.8,
            linestyles="solid")
ax = plt.gca()
font1 = {'family': 'Arial',
         'weight': 'normal',
